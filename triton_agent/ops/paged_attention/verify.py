"""PagedAttention: verify + benchmark."""
...
def verify_paged_attention(triton_output, ref_output, contract) -> CandidateResult:
    import torch
...
def benchmark_paged_attention(kernel_fn, inputs, contract, baseline_latency_us=0.0, warmup=20, repeat=100):
    import torch, time
    q, kc, vc, bt, cl = inputs
    for _ in range(warmup): kernel_fn(q, kc, vc, bt, cl)
    torch.cuda.synchronize()
    lats = []
    for _ in range(repeat):
        t0 = time.perf_counter(); kernel_fn(q, kc, vc, bt, cl); torch.cuda.synchronize()
